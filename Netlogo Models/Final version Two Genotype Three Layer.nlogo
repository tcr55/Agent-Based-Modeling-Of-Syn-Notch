...
nucleus-breed-own [
  ...
  cleaved-nuc-notch2-count-value
...
breed [cleaved-notch2-breed  cleaved-notch2]

cleaved-notch2-breed-own [
  birth       ;-- the tick that the original notch was created
  parent      ;-- owning nucleus
  parent-who  ;-- owning nucleus id
  time-cleaved;-- time the protein was cleaved and sent to nucleus
...
breed [notch2-nuc-breed  nucleus-notch2]
...
to setup
  ...
  set notch2-cleaved-diffusion-time-signal (notch-cleaved-diffusion-time / 4.0)
  ...
  set delta2-transcription-rate delta-transcription-initial-rate
  set notch2-transcription-rate notch-transcription-initial-rate
  ;;--------------------------------------------------
...
to go
  ...
  transform-proteins2 ; second transformation for the second notch pathway
...
to age-out-proteins
  ...
  ask cleaved-notch2-breed    with [ticks > ( birth + notchAge )] [ die ]
  ...
  ask notch2-nuc-breed        with [ticks > ( birth + notchAge )] [
    ...
    ask parent [
      set cleaved-nuc-notch2-count-value ( cleaved-nuc-notch2-count-value - 1 )
    ]
...
to transcribe-proteins
  ; to create two genotype here, it will randomly select nucleus to start, some will remain intert initially (ie A+B --> A+C )
  ; simple two layer circuit where only ones transcribing notch will activate the non transcribing cells,
  ;now the initial cell will cause cleaveage of notch1 which will triger the relase of delta2 to attach to notch2 on the original genotype 1 cells
  ; for simplicity the production parameters (rate intial) will be identical between Notch 1 and Notch 2
  let third-value (round ((cell-row-cnt * cell-col-cnt) / 2 ))


  ask n-of third-value nucleus-breed [

    set notch2-transcription-rate (notch-transcription-initial-rate + cleaved-nuc-notch-count-value)
...
;follow the same transformation as above but with the second notch signaling
to transform-proteins2
  ...
  ;;-----------------------------------------------------------------
  ;; ask all cleaved-notch agents to diffuse a certain amount
  ask cleaved-notch2-breed with [time-cleaved + indCleavedDiffTime < ticks] [
    ...
    ask local-lipid [
      ...
      if border-region != 0 [
         ...
         if ind-to-cleave != nobody [
           ...
           ask ind-to-cleave [
             set breed           cleaved-notch2-breed
             set shape           "square 2"
             set color           yellow
             set heading towards parent
             ...
             ;; establish the time to destination for the current cleaved agent
             set indCleavedDiffTime (random-normal notch-cleaved-diffusion-time notch2-cleaved-diffusion-time-signal)
           ]
         ]
      ]
    ]
...
to-report neuron-cnt
  ...
  ask nucleus-breed [
    ...
    if currentNuclearNotch2Cnt = 0 [
      set modifiedneuronCnt (modifiedneuronCnt + 1)
...
to check-cell-line
  ...
  ask nucleus-breed [
    ...
    set currentNuclearNotch2Cnt (count notch2-nuc-breed with [parent = myself])
      ...
      if count delta-breed with [parent = myself] > 5  and currentNuclearNotch2Cnt = 0 [
        ask nucleus-breed with [count delta-breed with [parent = myself] > 0  ] [
          ask patches in-radius 5.5 [ set pcolor blue] ]
      ]
      if  currentNuclearNotchCnt = 0
      [
        ask nucleus-breed with [currentNuclearNotchCnt = 0 and any? delta-breed with [parent = myself] = 0 ] [
    ask patches in-radius 5.5 [ set pcolor black]
      ...
      if currentNuclearNotchCnt != 0 [
        if cell-line-overlay? [
            ask nucleus-breed with [currentNuclearNotchCnt != 0] [
            ask patches in-radius 5.5 [ set pcolor green] ]
      ]
        ]

      if count delta-breed with [parent = myself] > 5 and currentNuclearNotch2Cnt > 5  [

        ask nucleus-breed with [count delta-breed with [parent = myself] > 0   and currentNuclearNotch2Cnt != 0 ] [
...
to color-cell-line
  ...
  ask nucleus-breed with [delta-breed with [parent = myself] != 0] [
    ask patches in-radius 5.5 [ set pcolor blue]
...
to color-cell-line-black

  ask nucleus-breed with [currentNuclearNotchCnt = 0 and notch-breed with [parent = myself] != 0] [
    ask patches in-radius 5.5 [ set pcolor black]
