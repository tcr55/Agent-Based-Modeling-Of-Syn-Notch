...
to production-setting
  ...
  ask nucleus-breed [ ifelse notch-production = true [ set notch-production true] [set notch-production false]]
end
...
to move-cell
 ...
 ; ]
  ...
  ask nucleus-breed with [cadherin = true] [
   ifelse count (nucleus-breed with [(self != myself) and cadherin = true]  in-radius (radius * 2)) > 0 [
   set binding true
     ask turtles with [move-who = [move-who] of myself] [ set binding true]
   ]
   [set binding false
     ask turtles with [move-who = [move-who] of myself] [ set binding false] ]
  ]
...
to transcribe-proteins
  ...
  ask nucleus-breed [
   ...
   ; if delta-transcription-rate >= random 50 [ ;100
   ;   hatch-delta-breed 1 [
   ;     set birth      ticks
    ;    set heading    (random 360)
     ;   set color      red
  ...
  ;]]
    ...
    ask nucleus-breed [ set currentNuclearNotchCnt (count notch-nuc-breed with [parent = myself])
        if currentNuclearNotchCnt > 3 [
          set cadherin true   ; simple cadherin activation (no agents set to caderin, it is a setting where if nuclear notch is above three the cadherin gene will be "on"
        ]
      ]
...
to transform-proteins
  ...
  ask lipid-breed [
    ...
    ask notch-breed in-radius (lipid-distance / 1) with [move-who = [move-who] of myself] [
  ...
  ask cleaved-notch-breed with [time-cleaved + indCleavedDiffTime < ticks] [
    ...
    if diffusion? [
        ask cleaved-notch-breed with [who = parent-who] [
       set compare parent-who
       ask nucleus-breed [
         if who = compare [
       set nucleus-match direction
       set nucleus-speed speed
         ]
       ]
       set heading nucleus-match
       forward nucleus-speed
       ]
    ]
...
to check-cell-line
  ...
  ask nucleus-breed [ set currentNuclearNotchCnt (count notch-nuc-breed with [parent = myself])
      ...
      if notch-production = true   [
        ask nucleus-breed with [notch-production = true] [
